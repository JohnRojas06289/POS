...
export default function OrdersPage() {
  ...
  const { data, isLoading } = useQuery({
    ...
    queryFn: () => posApi.getOrders({ limit: '100' }),
    retry: 1,
  });
            ...
            <tbody className="divide-y divide-[var(--border-default)]">
              {isLoading ? (
                Array.from({ length: 6 }).map((_, index) => (
                  <tr key={index}>
                    <td className="px-4 py-3" colSpan={5}><Skeleton variant="table-row" /></td>
                  </tr>
                ))
              ) : filtered.length > 0 ? (
                filtered.map((order: { id: string; status: string; total: number; customerId?: string | null; createdAt?: string }) => (
                ...
                <tr>
                  <td colSpan={5} className="px-4 py-8 text-center text-sm text-[var(--text-tertiary)]">No hay órdenes para mostrar</td>
                </tr>
              )}
            </tbody>
          </table>
